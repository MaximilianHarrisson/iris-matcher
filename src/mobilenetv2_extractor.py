...
class MobileNetV2Extractor:
    """https://docs.pytorch.org/vision/0.16/models/generated/torchvision.models.mobilenet_v2.html"""
    def __init__(self):
        resolved_device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.device = torch.device(resolved_device)

        self.feature_extractor, self.pooling = self._initialize_mobilenetv2()
    ...
    def _initialize_mobilenetv2(self) -> tuple:
    ...
    def extract(self, img_path: str) -> np.ndarray:
        try:
            ...
            raise RuntimeError(f'Failed to extract MobileNetV2 features from: {img_path}') from exc
